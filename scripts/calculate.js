...
function renderBaseFaq(result, comparison, numberLocale) {
  ...
  const expectedRows = expectationRows(comparison).map((row) => {
    ...
    return `<tr><th>${row.target}PU</th><td${chargeClass}>${row.charge.toFixed(2)}連</td><td${chargeClass}>${Math.round(row.chargePyroxene).toLocaleString(numberLocale)}</td><td${pointClass}>${row.point.toFixed(2)}連</td><td${pointClass}>${Math.round(row.pointPyroxene).toLocaleString(numberLocale)}</td></tr>`;
  }).join('');
...
function renderFaq(result, comparison, locale = 'ja') {
  ...
  return replaceExact(japanese, [
    ...
    ['<th rowspan="2">必要数</th>', '<th rowspan="2">PUs required</th>'],
    ['呼出チャージ', 'Recruitment Charge'],
    ['呼出ポイント', 'Recruitment Points'],
    ['期待募集', 'Expected pulls'],
    ['青輝石', 'Pyroxene'],
    ['連</td>', ' pulls</td>'],
    ['Pyroxeneは「Expected pulls回数 × 120」を四捨五入しています。期待値は長く繰り返したときの平均であり、その回数で必ず揃うという意味ではありません。', 'Pyroxene is the expected number of pulls multiplied by 120 and rounded to the nearest whole number. An expected value is a long-run average, not a guarantee that every PU will be obtained within that many pulls.'],
    ...
    ['Recruitment Chargeで1PUを100連以内', '1 PU within 100 pulls using Recruitment Charge'],
    ['それを2回掛けた単純計算', 'Simple calculation using two fixed 100-pull windows'],
    ['200連を通して計算した実際の確率', 'Exact probability across all 200 pulls'],
    ['固定区切りでは拾えない確率', 'Probability missed by fixed windows'],
