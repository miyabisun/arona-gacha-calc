...
function renderBaseFaq(result, comparison, numberLocale) {
  const oneWithin100 = result.simpleComparison.oneWithin100;
  const expectedRows = expectationRows(comparison).map((row) => {
    const chargeClass = row.charge <= row.point ? ' class="best"' : '';
    const pointClass = row.point <= row.charge ? ' class="best"' : '';
    return `<tr><th>${row.target}PU</th><td${chargeClass}><span class="metric">${row.charge.toFixed(2)}連</span><span class="metric">${Math.round(row.chargePyroxene).toLocaleString(numberLocale)}</span><span class="metric-unit">青輝石</span></td><td${pointClass}><span class="metric">${row.point.toFixed(2)}連</span><span class="metric">${Math.round(row.pointPyroxene).toLocaleString(numberLocale)}</span><span class="metric-unit">青輝石</span></td></tr>`;
  }).join('');
...
:root{color-scheme:light dark;--surface:#faf6ef;--raised:#fffdf8;--on:#3a2f28;--muted:#6f6257;--border:#e3d9c9;--accent:#9a6a00;--accent-subtle:rgba(154,106,0,.10);--link:#14506e}*{box-sizing:border-box}html{background:var(--surface)}body{margin:0;background:var(--surface);color:var(--on);font-family:system-ui,sans-serif;font-size:16px;line-height:1.6}a{color:var(--link)}a:focus-visible{outline:2px solid var(--accent);outline-offset:2px}main{width:min(1100px,calc(100% - 24px));margin:24px auto}.nav{display:flex;gap:16px;margin-bottom:16px;border-bottom:1px solid var(--border)}.nav a{padding:8px 4px;color:var(--muted);font-size:15px;font-weight:500;text-decoration:none}.nav a[aria-current]{color:var(--on);border-bottom:2px solid var(--accent)}header,.qa{background:var(--raised);border:1px solid var(--border);border-radius:8px;padding:16px;margin:0 0 16px}.header-row{display:flex;align-items:center;justify-content:space-between;gap:16px}h1{font-size:17px;font-weight:600;line-height:1.3;margin:0}.repo-link{display:inline-flex;align-items:center;gap:8px;color:var(--muted);font-size:12px;text-decoration:none}.repo-link:hover{color:var(--link)}.repo-link svg{width:20px;height:20px;flex:none}.lead{color:var(--muted);margin-bottom:0}.qa h2{font-size:17px;line-height:1.3;margin:0 0 12px;padding:4px 0 4px 12px;border-left:4px solid var(--accent)}.answer{color:var(--accent);font-weight:700}.table-wrap{overflow:visible}table{width:100%;border-collapse:collapse;font-variant-numeric:tabular-nums;white-space:nowrap}.expectation-table{table-layout:fixed}.expectation-table th:first-child{width:20%}.expectation-table th:not(:first-child),.expectation-table td{text-align:center}.metric,.metric-unit{display:block;white-space:nowrap}.metric-unit{color:var(--muted);font-size:12px}.comparison-table{table-layout:fixed;white-space:normal}.comparison-table th{width:76%;overflow-wrap:anywhere}.comparison-table td{width:24%;white-space:nowrap}th,td{padding:8px;border-bottom:1px solid var(--border);text-align:right}th{text-align:left}thead th{text-align:center;color:var(--muted);font-size:14px}thead th:first-child{text-align:left}.best{color:var(--link);font-weight:600}code{background:var(--accent-subtle);color:var(--on);border-radius:6px;padding:4px}.note{color:var(--muted);font-size:14px}footer{text-align:center;color:var(--muted);font-size:12px;margin-top:24px}@media(max-width:760px){main{width:min(100% - 16px,1100px);margin:16px auto}header,.qa{padding:12px}.qa{overflow-wrap:anywhere}.repo-link span{display:none}th,td{padding:8px 4px;font-size:14px}}@media(prefers-color-scheme:dark){:root{--surface:#191919;--raised:#232323;--on:#e6e6e6;--muted:#9a9a9a;--border:#333333;--accent:#e0a800;--accent-subtle:rgba(224,168,0,.15);--link:#7fdbff}}@media(prefers-reduced-motion:reduce){*,*:before,*:after{scroll-behavior:auto!important;transition-duration:.01ms!important;animation-duration:.01ms!important;animation-iteration-count:1!important}}
</style></head><body><main><nav class="nav"><a href="./">確率表</a><a href="faq.html" aria-current="page">Q&amp;A</a></nav><header><div class="header-row"><h1>計算についてのQ&amp;A</h1><a class="repo-link" href="https://github.com/miyabisun/arona-gacha-calc" aria-label="GitHubリポジトリを開く"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3.3-.4 6.8-1.6 6.8-7A5.4 5.4 0 0 0 19.4 4 5 5 0 0 0 19.3.5S18.2.1 15 1.8a13.4 13.4 0 0 0-7 0C4.8.1 3.7.5 3.7.5A5 5 0 0 0 3.6 4a5.4 5.4 0 0 0-1.4 3.7c0 5.4 3.5 6.5 6.8 7A4.8 4.8 0 0 0 8 18v4"/><path d="M8 19c-3 .9-3-1.5-4-2"/></svg><span>miyabisun/arona-gacha-calc</span></a></div><p class="lead">グラフの読み方と、呼び出しポイント・呼び出しチャージの計算方法です。</p></header>
<section class="qa"><h2>結局1度の募集で、何連が期待値なの？</h2><div class="table-wrap"><table class="expectation-table"><thead><tr><th>必要数</th><th>チャージ</th><th>ポイント</th></tr></thead><tbody>${expectedRows}</tbody></table></div><p class="note">青輝石は「期待募集回数 × 120」を四捨五入しています。期待値は長く繰り返したときの平均であり、その回数で必ず揃うという意味ではありません。</p></section>
<section class="qa"><h2>呼び出しチャージの2PUを200連で引く確率は56.34%では？</h2><table class="comparison-table"><tbody><tr><th>呼出チャージで1PUを100連以内</th><td>${pct(oneWithin100)}</td></tr><tr><th>それを2回掛けた単純計算</th><td>${pct(result.simpleComparison.twoFixedWindows)}</td></tr><tr><th>200連を通して計算した実際の確率</th><td class="answer">${pct(result.summary['200'].newTwoPUs)}</td></tr><tr><th>固定区切りでは拾えない確率</th><td class="answer">+${pct(result.simpleComparison.recoveredByFlexibleSplit)}</td></tr></tbody></table><p>56.34%は、各PUへ100連ずつ固定で割り当てた計算です。実際には1PU目を早く引いた分だけ2PU目へ多く回せるため、その経路をすべて含めると64.09%になります。</p></section>
<section class="qa"><h2>グラフは何を比較している？</h2><p>必要なPU数を指定し、各募集回数までに全員を揃えられる累積確率を、呼び出しポイントと呼び出しチャージで比較しています。乱数による近似ではなく1連ごとの厳密計算で、別の計算方法とも一致することを確認しています。</p></section>
<section class="qa"><h2>どこまでをPU獲得として数える？</h2><p>PU出現率は0.7%として計算しています。</p></section>
<section class="qa"><h2>厳密計算の組み合わせ爆発はしない？</h2><p><strong>呼び出しチャージ：</strong>まず、1PUを引くまでに1〜200連のそれぞれで終わる確率を作ります。その分布を必要なPU数だけ順番に重ね、合計募集回数以下になる確率を足します。</p><p><strong>呼び出しポイント：</strong>募集回数ごとに、自力で引けたPU数の確率を求めます。そこへ200連ごとに交換できるPU数を足し、必要数以上になった確率を合計します。</p><p>履歴そのものを列挙せず、同じ状態をまとめて計算するため、10PU・2000連でも現実的な時間で完了します。実装は <a href="https://github.com/miyabisun/arona-gacha-calc/blob/main/scripts/compare.js">scripts/compare.js</a> で確認できます。</p></section><footer>Generated by scripts/calculate.js</footer></main></body></html>`;
}

function renderFaq(result, comparison, locale = 'ja') {
  const numberLocale = locale === 'en' ? 'en-US' : 'ja-JP';
  const japanese = localizeShell(renderBaseFaq(result, comparison, numberLocale), locale, 'faq');
  if (locale === 'ja') return japanese;
  if (locale !== 'en') throw new Error(`Unsupported locale: ${locale}`);
  return replaceExact(japanese, [
    ['<html lang="ja">', '<html lang="en">'],
    ['<title>募集確率 Q&amp;A</title>', '<title>Recruitment Probability Q&amp;A</title>'],
    ['<h1>計算についてのQ&amp;A</h1>', '<h1>Recruitment Probability Q&amp;A</h1>'],
    ['aria-label="GitHubリポジトリを開く"', 'aria-label="Open the GitHub repository"'],
    ['グラフの読み方と、呼び出しポイント・呼び出しチャージの計算方法です。', 'How to read the chart and how Recruitment Points and Recruitment Charge are calculated.'],
    ['結局1度の募集で、何連が期待値なの？', 'How many pulls should I expect to need per banner?'],
    ['<th>必要数</th>', '<th>PUs</th>'],
    ['<th>チャージ</th>', '<th>Charge</th>'],
    ['<th>ポイント</th>', '<th>Points</th>'],
    ['呼び出しチャージ', 'Recruitment Charge'],
    ['呼出チャージ', 'Recruitment Charge'],
    ['呼び出しポイント', 'Recruitment Points'],
    ['青輝石', 'Pyroxene'],
    ['連</span>', ' pulls</span>'],
    ['Pyroxeneは「期待募集回数 × 120」を四捨五入しています。期待値は長く繰り返したときの平均であり、その回数で必ず揃うという意味ではありません。', 'Pyroxene is the expected number of pulls multiplied by 120 and rounded to the nearest whole number. An expected value is a long-run average, not a guarantee that every PU will be obtained within that many pulls.'],
    ['<th>1PU</th>', '<th>1 PU</th>'],
    ['<th>2PU</th>', '<th>2 PUs</th>'],
    ['<th>3PU</th>', '<th>3 PUs</th>'],
    ['<th>4PU</th>', '<th>4 PUs</th>'],
    ['Recruitment Chargeの2PUを200連で引く確率は56.34%では？', 'Isn’t the chance of obtaining 2 PUs within 200 pulls using Recruitment Charge 56.34%?'],
    ['Recruitment Chargeで1PUを100連以内', '1 PU in 100 pulls'],
    ['それを2回掛けた単純計算', 'Two fixed 100-pull windows'],
    ['200連を通して計算した実際の確率', 'Exact 200-pull probability'],
    ['固定区切りでは拾えない確率', 'Missed by fixed windows'],
    ['56.34%は、各PUへ100連ずつ固定で割り当てた計算です。実際には1PU目を早く引いた分だけ2PU目へ多く回せるため、その経路をすべて含めると64.09%になります。', 'The 56.34% figure assigns a fixed block of 100 pulls to each PU. If the first PU arrives early, the remaining pulls can be used for the second. Including every such path raises the exact probability to 64.09%.'],
    ['グラフは何を比較している？', 'What does the chart compare?'],
    ['必要なPU数を指定し、各募集回数までに全員を揃えられる累積確率を、Recruitment PointsとRecruitment Chargeで比較しています。乱数による近似ではなく1連ごとの厳密計算で、別の計算方法とも一致することを確認しています。', 'For a selected number of PUs, the chart compares the cumulative probability of obtaining all of them by each pull count under Recruitment Points and Recruitment Charge. The values are calculated exactly for every pull—not estimated by random sampling—and cross-checked with independent formulas.'],
    ['どこまでをPU獲得として数える？', 'What counts as obtaining a PU?'],
    ['PU出現率は0.7%として計算しています。', 'The calculation uses a 0.7% appearance rate for the targeted PU.'],
    ['厳密計算の組み合わせ爆発はしない？', 'Does exact calculation cause a combinatorial explosion?'],
    ['<strong>Recruitment Charge：</strong>まず、1PUを引くまでに1〜200連のそれぞれで終わる確率を作ります。その分布を必要なPU数だけ順番に重ね、合計募集回数以下になる確率を足します。', '<strong>Recruitment Charge:</strong> First, calculate the probability that one PU is obtained on each pull from 1 through 200. Convolve that distribution once per required PU, then sum the probabilities whose total pull count is within the selected limit.'],
    ['<strong>Recruitment Points：</strong>募集回数ごとに、自力で引けたPU数の確率を求めます。そこへ200連ごとに交換できるPU数を足し、必要数以上になった確率を合計します。', '<strong>Recruitment Points:</strong> For each pull count, calculate the probability distribution of PUs obtained naturally. Add one exchangeable PU per 200 pulls, then sum the probability of meeting or exceeding the required count.'],
    ['履歴そのものを列挙せず、同じ状態をまとめて計算するため、10PU・2000連でも現実的な時間で完了します。実装は ', 'Equivalent states are aggregated instead of enumerating every pull history, so the calculation remains practical for 10 PUs and 2,000 pulls. The implementation is available in '],
    [' で確認できます。', '.'],
  ]);
}
...
function main() {
  ...
  fs.writeFileSync(path.join(OUTPUT_DIR, 'en', 'faq.html'), renderFaq(result, comparison, 'en'));
...
module.exports = { calculate, puRate, singleCycleDistribution, stateDynamicProgram, convolutionCompletion, expectedPulls, expectationRows, renderFaq };
