...
function renderFaq(result, comparison) {
  ...
  const expectedRows = expectationRows(comparison).map((row) => {
    ...
    return `<tr><th>${row.target}PU</th><td${chargeClass}>${row.charge.toFixed(2)}連</td><td${chargeClass}>${Math.round(row.chargePyroxene).toLocaleString('ja-JP')}</td><td${pointClass}>${row.point.toFixed(2)}連</td><td${pointClass}>${Math.round(row.pointPyroxene).toLocaleString('ja-JP')}</td></tr>`;
  }).join('');
