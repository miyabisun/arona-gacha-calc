...
// 募集回数特典(公式告知679): 累計回数の閾値で期間限定10連チケットを獲得し、即時使用する。
// チケットで回した10連も累計回数と呼び出しチャージに加算される(実践値比較の前提)。
const TICKET_THRESHOLDS = [70, 130, 150, 170, 270, 330, 350, 370];
const TICKET_PULLS = 10;

/** 持出連(石で支払った連数)から、チケット込みの累計募集回数を返す。 */
function totalWithTickets(paid) {
  let tickets = 0;
  for (;;) {
    const total = paid + tickets * TICKET_PULLS;
    const count = TICKET_THRESHOLDS.filter((threshold) => threshold <= total).length;
    if (count === tickets) return total;
    tickets = count;
  }
}

/** 新仕様の理論値曲線を持出連の軸へ引き直す。特典到達のたびに崖ができる。 */
function practicalChargeCurves(result) {
  return Object.fromEntries(TARGETS.map((target) => [target,
    Array.from({ length: target * 200 + 1 }, (_, paid) => result.curves.anniversary5_5[target][Math.min(totalWithTickets(paid), target * 200)]),
  ]));
}
...
function graphGeometry(result, target) {
  ...
  const yLines = [0, .25, .5, .75, 1].map((value) => {
    const y = top + (1 - value) * plotHeight;
    return `<line class="horizontal" x1="${left}" y1="${y}" x2="${width - right}" y2="${y}"/><text class="y-label" x="${left - 10}" y="${y + 4}">${value * 100}%</text>`;
  }).join('');
  const xLines = Array.from({ length: Math.floor(maxPulls / 10) + 1 }, (_, index) => index * 10).map((pull) => {
    const x = left + (pull / maxPulls) * plotWidth;
    const major = pull % 100 === 0;
    return `<line class="${major ? 'major' : 'minor'}" x1="${x}" y1="${top}" x2="${x}" y2="${height - bottom}"/>${major ? `<text class="x-label" x="${x}" y="${height - 14}">${pull}</text>` : ''}`;
  }).join('');
...
function standaloneSvg(result, target) {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 920 430" role="img" aria-labelledby="title desc"><title id="title">${target}PUのガチャ確率</title><desc id="desc">呼出チャージは実線、呼出ポイントは破線。0%から100%までの累積確率。</desc><style>:root{color-scheme:light dark}.grid line{stroke:#e3d9c9}.grid .minor{stroke-width:.6}.grid .major{stroke:#a99c8e}.grid text{fill:#6f6257;font:12px system-ui,sans-serif;text-anchor:middle}.grid .y-label{text-anchor:end}.curve{fill:none;stroke-width:4;stroke-linejoin:round;stroke-linecap:round}.charge{stroke:#14506e}.point{stroke:#9a6a00;stroke-dasharray:10 7}.hit,.hover-dot{display:none}@media(prefers-color-scheme:dark){.grid line{stroke:#333}.grid .major{stroke:#666}.grid text{fill:#9a9a9a}.charge{stroke:#7fdbff}.point{stroke:#e0a800}}</style>${graphGeometry(result, target)}</svg>`;
}

function renderJapaneseHtml(result, page = 'index') {
  const practical = page === 'index';
  const chargeCurves = practical ? practicalChargeCurves(result) : null;
  const chartData = JSON.stringify(Object.fromEntries(TARGETS.map((target) => [target, {
    charge: practical ? chargeCurves[target] : result.curves.anniversary5_5[target].slice(0, target * 200 + 1),
    point: result.curves.anniversary1_0[target].slice(0, target * 200 + 1),
  }]))).replaceAll('<', '\\u003c');
  const heading = practical ? 'BlueArchive ガチャ実践値比較' : 'BlueArchive ガチャ確率表';
  const axisPrefix = practical ? '持出' : '';
  const tipPrefix = practical ? "'持出'+" : '';
  const premise = practical ? '<section class="panel"><h2>実践値の前提</h2><ul class="rules"><li>募集回数特典の累計<b>70・130・150・170回</b>（2周目は270・330・350・370回）で期間限定10連チケットを獲得し、即座に使う。</li><li>チケットで回した10連分も累計募集回数と呼び出しチャージに加算される。</li><li>横軸は<b>持出連</b>＝石で支払った募集回数。チケット分は含まれないため、新仕様の曲線は特典に到達するたび崖のように伸びる（持出70連で累計80連ぶんへ）。</li><li>旧仕様（呼出ポイント）に募集回数特典はないため理論値のまま。</li></ul></section>' : '';
...
function renderHtml(result, locale = 'ja', page = 'index') {
  ...
  const pairs = [
    ...
    ['aria-label="必要なPU数"', 'aria-label="Number of PUs required"'],
    ...
    ['target+\'PUを揃える呼出ポイントと呼出チャージの累積確率', 'target+(target===1?\' PU\':\' PUs\')+\': cumulative probability for Recruitment Points and Recruitment Charge'],
    ["'%\\n呼出ポイント '", "'%\\nRecruitment Points '"],
  ];
  if (practical) {
    pairs.push(
      ['<title>BlueArchive ガチャ実践値比較</title>', '<title>Blue Archive Practical Recruitment Chart</title>'],
      ['<h1>BlueArchive ガチャ実践値比較</h1>', '<h1>Blue Archive Practical Recruitment Chart</h1>'],
      ['<h2 id="chart-title">2PUが揃う確率（持出1〜400連）</h2>', '<h2 id="chart-title">Probability of obtaining 2 PUs (1–400 paid pulls)</h2>'],
      ["function chartTitle(target){return target===1?'1PUを獲得できる確率（持出1〜200連）':target+'PUが揃う確率（持出1〜'+(target*200)+'連）'}", "function chartTitle(target){return 'Probability of obtaining '+target+(target===1?' PU':' PUs')+' (1–'+(target*200)+' paid pulls)'}"],
      ["'持出'+current+'連\\n呼出チャージ '", "'Paid '+current+' pulls\\nRecruitment Charge '"],
      ['<h2>実践値の前提</h2>', '<h2>Assumptions</h2>'],
      ['募集回数特典の累計<b>70・130・150・170回</b>（2周目は270・330・350・370回）で期間限定10連チケットを獲得し、即座に使う。', 'The recruit-count rewards grant a limited ten-pull ticket at <b>70, 130, 150, and 170</b> cumulative pulls (270, 330, 350, and 370 on the second lap), used immediately.'],
      ['チケットで回した10連分も累計募集回数と呼び出しチャージに加算される。', 'Ticket pulls also count toward the cumulative total and Recruitment Charge.'],
      ['横軸は<b>持出連</b>＝石で支払った募集回数。チケット分は含まれないため、新仕様の曲線は特典に到達するたび崖のように伸びる（持出70連で累計80連ぶんへ）。', 'The x-axis is <b>paid pulls</b> — recruitments paid with Pyroxene. Ticket pulls are excluded, so the new-system curve jumps like a cliff at each reward (70 paid pulls counts as 80 pulls).'],
      ['旧仕様（呼出ポイント）に募集回数特典はないため理論値のまま。', 'The old system (Recruitment Points) had no recruit-count rewards, so its curve stays theoretical.'],
    );
  } else {
    pairs.push(
      ['<title>BlueArchive ガチャ確率表</title>', '<title>Blue Archive Recruitment Probability Chart</title>'],
      ['<h1>BlueArchive ガチャ確率表</h1>', '<h1>Blue Archive Recruitment Probability Chart</h1>'],
      ['<h2 id="chart-title">2PUが揃う確率（1〜400連）</h2>', '<h2 id="chart-title">Probability of obtaining 2 PUs (1–400 pulls)</h2>'],
      ["function chartTitle(target){return target===1?'1PUを獲得できる確率（1〜200連）':target+'PUが揃う確率（1〜'+(target*200)+'連）'}", "function chartTitle(target){return 'Probability of obtaining '+target+(target===1?' PU':' PUs')+' (1–'+(target*200)+' pulls)'}"],
      ["current+'連\\n呼出チャージ '", "current+' pulls\\nRecruitment Charge '"],
    );
  }
  return replaceExact(japanese, pairs);
}
...
function main() {
  ...
  fs.writeFileSync(path.join(OUTPUT_DIR, 'en', 'index.html'), renderHtml(result, 'en'));
  fs.writeFileSync(path.join(OUTPUT_DIR, 'theory.html'), renderHtml(result, 'ja', 'theory'));
  fs.writeFileSync(path.join(OUTPUT_DIR, 'en', 'theory.html'), renderHtml(result, 'en', 'theory'));
...
module.exports = { calculateComparison, rateForCharge, singleCycleDistribution, fivePointFiveCurves, convolutionCurves, onePointZeroCurves, onePointZeroClosedFormCurves, binomialTail, totalWithTickets, practicalChargeCurves, renderHtml, standaloneSvg };
